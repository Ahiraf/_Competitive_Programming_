//This note is from Abhishek Saini sir
...
void kodiko()
{
   // STL is a tool
    ...
    // n = 1e5
    // the formula of total pairs = n * (n + 1) / 2
    long long int ans = 0;
    map<int, int> mp;
    int n;
    vector<int> A(n);
...
//Time Complexity : O(nlog(n))
    ...
    // pairs with A[i] == A[j], Let these pairs = 500
    // total number of pairs = nC2 = (n - 1) * (n) / 2 
    //total number of pairs - 500 = not equal pairs
    // 0 + 1 + 2 + 3......... n - 1 (for an array of length n)
    // if j = 0, 0 elements before this
    // if j = 1, 1 
    // if j = 2, 2
    ...
    // X elements
    // total number of pairs = XC2 = X * (X - 1) / 2

    // n elements ..How many pairs can we make by these n elements ? = nC2
    // Total number of pairs = nC2 = n * (n - 1) / 2

    for(int k = 0; k < n; k++) {
        int freq = mp[A[k]];

        ans = ans + freq * (freq - 1) / 2; //Applying n*(n-1)/2
