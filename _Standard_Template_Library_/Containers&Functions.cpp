//I attended Indian Competitive Programming Summer Camp and got this basics from Abhishek Saini Sir
...
int main(){
    ...
    cout << newVec[0]; // 0
     //If I want to insert a value (500) after 12 time complexity will be O(n)
     //because there was not space for that 500 ...other values after 12 has to be shifted and then 500 can be assigned
    // [1, 4, 5, 10, 12, [500] 15....]
...
//So to avoid error we can avoid iterator
//or can use if (it !=s.end())
    ...
    // mp.size() = 3
    cout << mp["yellow"] << "\n"; // 0....
    //Because the operation of square bracket is that if the key is not present, it will first assign the key then calculate its value
    // mp.size() = 4


//So again to avoid this peculiar issue we can use if....like following : 
